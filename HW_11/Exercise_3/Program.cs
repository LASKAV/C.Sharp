...
/*
                Задание 3
Создаейте приложение эмулирющее очерь в популяром кафе.
Посетители кафе приходят и поппадают в очередь если нет свободного
места При освобождении столика в кафе первый поститель очери занимает его
Если приходит клиент который резервировал столик
на определенное время он получает зарезервированный столик вне очереди 
...
class Program
{
    ...
    {
        // Добавляем несколько столиков в кафе
